...
export default function ImageStudioModule({
  results, roundCount, pendingSave, isLocallySaved, onSave, onCreateVideo, onPreview, onRetryFailed, retrying, embedded = false,
}: {
  results: ImageResult[]
  roundCount: number
  ...
  return <section className={embedded ? "min-w-0" : "relative z-10 mx-auto mt-8 max-w-[1080px]"} aria-label="图片生成结果">
    ...
    <div className={`grid gap-4 ${results.length === 1 ? "grid-cols-1" : "sm:grid-cols-2"}`}>
      {results.map((result) => {
        const saving = pendingSave?.(result) ?? false
        const saved = isLocallySaved?.(result) ?? result.output.delivery_status === "local"
        return <article key={`${result.generationItemId}:${result.outputIndex}`} className="overflow-hidden rounded-xl border border-white/[0.1] bg-[#202127]">
          <div className={`${embedded ? "h-[min(46vh,420px)]" : "aspect-square"} bg-[#17181e]`}>{result.src ? <button type="button" onClick={() => onPreview?.(result)} disabled={!onPreview} title="预览图片" className="h-full w-full cursor-zoom-in disabled:cursor-default"><img src={result.src} alt={result.output.label} className="h-full w-full object-contain" /></button> : <div className="grid h-full place-items-center text-[#777781]"><HardDrive size={20} /></div>}</div>
          <div className="flex min-h-12 flex-wrap items-center justify-between gap-2 px-3 py-2.5">
            ...
            <div className="flex items-center gap-2">
              <button type="button" disabled={saving || saved || !result.output.download_url} onClick={() => onSave(result)} className="flex h-8 items-center gap-1.5 rounded-md border border-white/10 px-2.5 text-xs text-[#d7d7dc] disabled:opacity-45">{saving ? <LoaderCircle className="animate-spin" size={13} /> : saved ? <Check size={13} /> : <HardDrive size={13} />}{saving ? "保存中" : saved ? "已保存" : "保存"}</button>
              <button type="button" disabled={saving} onClick={() => onCreateVideo(result)} className="flex h-8 items-center gap-1.5 rounded-md bg-[#7047f6] px-2.5 text-xs text-white disabled:opacity-45"><Play size={13} />生成视频</button>
            </div>
          </div>
        </article>
      })}
