...
function PasswordField({
  ...
  compact = false, containerClassName = "",
}: {
  id: string
  label: string
  value: string
  ...
  return <div className={containerClassName}>
    <label htmlFor={id} className={compact ? "block text-xs text-[#bcbcc4]" : "block text-sm text-[#d4d4da]"}>{label}</label>
    <div className={`relative ${compact ? "mt-1.5" : "mt-2"}`}>
      <input
        ...
        autoFocus={autoFocus}
        className={`${compact ? "h-10 bg-[#212228] text-sm" : "h-11 bg-[#202127] text-white"} w-full rounded-lg border border-white/10 px-3 pr-12 outline-none transition focus:border-[#8d75ff] focus:ring-2 focus:ring-[#7655ff]/20`}
      />
      <button
        type="button"
        ...
        title={toggleLabel}
        className="absolute inset-y-1 right-1 grid aspect-square place-items-center rounded-md text-[#85858f] transition hover:bg-white/[0.06] hover:text-[#dedee5] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#927cff]"
      >
...
function AdminUsers({ user, csrfToken, onBack }: { user: User; csrfToken: string; onBack: () => void }) {
  ...
  return <div className="min-h-screen bg-[#17181e] text-[#f5f5f7]">
    <header className="flex h-[68px] items-center justify-between border-b border-white/[0.08] bg-[#111217] px-4 sm:px-6">
      <div className="flex items-center gap-3"><button onClick={onBack} title="返回创作台" className="grid size-9 place-items-center rounded-lg text-[#b7b7c0] hover:bg-white/[0.06] hover:text-white"><ArrowLeft size={18} /></button><span className="h-6 w-px bg-white/10" /><Users size={18} className="text-[#a58fff]" /><h1 className="text-base font-medium">管理设置</h1></div>
      <div className="text-right"><p className="text-sm">{user.display_name}</p><p className="text-[11px] text-[#85858f]">{roleLabel[user.role]}</p></div>
    </header>
    <div className="mx-auto max-w-[1180px] px-5 pt-3 lg:px-8"><Tabs activeKey={adminTab} onChange={setAdminTab} items={[{ key: "accounts", label: "账号管理" }, ...(user.role === "super_admin" ? [{ key: "providers", label: "AI 供应商" }, { key: "storage", label: "媒体存储" }] : [])]} /></div>
    {adminTab === "accounts" ? <main className="mx-auto grid max-w-[1180px] gap-10 px-5 py-8 lg:grid-cols-[300px_minmax(0,1fr)] lg:px-8">
      <section>
        <div className="mb-6"><span className="grid size-10 place-items-center rounded-lg bg-[#7655ff]/15 text-[#a58fff]"><UserPlus size={19} /></span><h2 className="mt-4 text-lg font-semibold">分配新账号</h2><p className="mt-1 text-sm text-[#8f8f99]">员工首次登录后必须修改初始密码。</p></div>
        <form onSubmit={(event) => { event.preventDefault(); createMutation.mutate() }} className="space-y-4">
          <label className="block text-xs text-[#bcbcc4]">登录账号<input value={username} onChange={(event) => setUsername(event.target.value)} className="mt-1.5 h-10 w-full rounded-lg border border-white/10 bg-[#212228] px-3 text-sm outline-none focus:border-[#8c74ff]" /></label>
          <label className="block text-xs text-[#bcbcc4]">员工姓名<input value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="mt-1.5 h-10 w-full rounded-lg border border-white/10 bg-[#212228] px-3 text-sm outline-none focus:border-[#8c74ff]" /></label>
          <PasswordField id="create-user-password" label="初始密码" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" minLength={PASSWORD_MIN_LENGTH} compact />
          <label className="block text-xs text-[#bcbcc4]">角色<select value={role} onChange={(event) => setRole(event.target.value as UserRole)} className="mt-1.5 h-10 w-full rounded-lg border border-white/10 bg-[#212228] px-3 text-sm outline-none focus:border-[#8c74ff]"><option value="employee">员工</option>{user.role === "super_admin" ? <><option value="admin">管理员</option><option value="super_admin">超级管理员</option></> : null}</select></label>
          {createMutation.isError ? <p className="text-sm text-red-300">{createMutation.error.message}</p> : null}
          <button disabled={createMutation.isPending || !username || !displayName || password.length < PASSWORD_MIN_LENGTH} className="flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-[#7047f6] text-sm font-medium disabled:opacity-40">{createMutation.isPending ? <LoaderCircle className="animate-spin" size={16} /> : <UserPlus size={16} />}创建账号</button>
        </form>
      </section>
      <section className="min-w-0">
        <div className="mb-4 flex items-end justify-between"><div><h2 className="text-lg font-semibold">员工账号</h2><p className="mt-1 text-sm text-[#8f8f99]">共 {usersQuery.data?.length ?? 0} 个账号</p></div></div>
        <div className="overflow-x-auto border-y border-white/[0.08]">
          <table className="w-full min-w-[680px] text-left text-sm">
            <thead className="text-xs text-[#85858f]"><tr><th className="px-3 py-3 font-medium">员工</th><th className="px-3 py-3 font-medium">角色</th><th className="px-3 py-3 font-medium">状态</th><th className="px-3 py-3 font-medium">最近登录</th><th className="px-3 py-3 text-right font-medium">操作</th></tr></thead>
            <tbody className="divide-y divide-white/[0.06]">
              {(usersQuery.data ?? []).map((account) => <tr key={account.id} className="hover:bg-white/[0.025]">
                <td className="px-3 py-3"><p className="font-medium text-[#ececf0]">{account.display_name}</p><p className="mt-0.5 text-xs text-[#83838d]">{account.username}{account.must_change_password ? " · 待修改初始密码" : ""}</p></td>
                <td className="px-3 py-3"><select disabled={account.id === user.id || (user.role === "admin" && account.role !== "employee")} value={account.role} onChange={(event) => updateMutation.mutate({ id: account.id, body: { role: event.target.value as UserRole } })} className="h-8 rounded-md border border-white/10 bg-[#202127] px-2 text-xs disabled:opacity-50"><option value="employee">员工</option>{user.role === "super_admin" ? <><option value="admin">管理员</option><option value="super_admin">超级管理员</option></> : null}</select></td>
                <td className="px-3 py-3"><span className={account.is_active ? "text-emerald-300" : "text-[#777781]"}>{account.is_active ? "已启用" : "已停用"}</span></td>
                <td className="px-3 py-3 text-xs text-[#9a9aa4]">{account.last_login_at ? new Date(account.last_login_at).toLocaleString("zh-CN", { hour12: false }) : "尚未登录"}</td>
                <td className="px-3 py-3 text-right"><div className="flex justify-end gap-2"><button onClick={() => { setResettingUserId(account.id); setResetPassword("") }} className="h-8 rounded-md px-2.5 text-xs text-[#b8a8ff] hover:bg-white/[0.06]">重置密码</button><button disabled={account.id === user.id || (user.role === "admin" && account.role !== "employee")} onClick={() => updateMutation.mutate({ id: account.id, body: { is_active: !account.is_active } })} className="h-8 rounded-md px-2.5 text-xs text-[#bdbdc5] hover:bg-white/[0.06] disabled:opacity-40">{account.is_active ? "停用" : "启用"}</button></div></td>
              </tr>)}
            </tbody>
          </table>
          {usersQuery.isLoading ? <p className="py-12 text-center text-sm text-[#85858f]">正在读取账号</p> : null}
        </div>
        {resettingUserId ? <form onSubmit={(event) => { event.preventDefault(); resetMutation.mutate(resettingUserId) }} className="mt-5 flex flex-col gap-3 border-t border-white/[0.08] pt-5 sm:flex-row sm:items-end"><PasswordField id="reset-user-password" label="新的初始密码" value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} autoComplete="new-password" minLength={PASSWORD_MIN_LENGTH} autoFocus compact containerClassName="min-w-0 flex-1" /><button disabled={resetPassword.length < PASSWORD_MIN_LENGTH || resetMutation.isPending} className="h-10 rounded-lg bg-[#7047f6] px-4 text-sm disabled:opacity-40">确认重置</button><button type="button" onClick={() => setResettingUserId(undefined)} className="h-10 rounded-lg px-4 text-sm text-[#aaaab3] hover:bg-white/[0.05]">取消</button></form> : null}
        {(updateMutation.isError || resetMutation.isError) ? <p className="mt-4 text-sm text-red-300">{updateMutation.error?.message ?? resetMutation.error?.message}</p> : null}
      </section>
    </main> : <Suspense fallback={<CenteredStatus label="正在加载设置" />}>{adminTab === "providers" ? <GrsProviderSettings csrfToken={csrfToken} /> : <QiniuStorageSettings csrfToken={csrfToken} />}</Suspense>}
  </div>
}
