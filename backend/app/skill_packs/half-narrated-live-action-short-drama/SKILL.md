...
> 映射：Design Image 2 → GRS；成片走本地 MiniMax H3 R2V，只送角色卡、场景卡和三联**起幅**裁切，禁止把整张 16:9 三联当 `<Picture n>`（本地 H3 会把三格插值成一条画面）；角色卡是多视图设定板，只锁身份/发型/服装，禁止把分格、白底、重复小人带进镜头；中格/右格用分秒文字演。出片前由用户确认 **2K 或 768P**（整集统一），单镜 **5–15** 整数秒。第 10–11 步 Seed Audio 独立旁白轨与字幕本期不接线。导台2 阶段仍是 script → assets → episodes → storyboard，只注入本包约束。
