...
OPERATION_DETAILS: dict[tuple[str, str], str] = {
    ...
    ("post", "/api/projects/{project_id}/episodes/{episode_id}/beats/{beat_id}/h3-prompt"): "需要登录和 X-CSRF-Token。为当前分镜入队 h3_prompt 任务，把 Beat 的 visual_prompt / audio / video_prompt_zh 一并交给模型装箱（会剥掉动作里整段粘贴的台词，内心/旁白不作为开口对白）。先按对白+调度把 video_duration 抬到能演完，再按官方 h3-prompt-writing 与导演台1 同款 timing 预算装箱六段 Ref2VA。英文须达到可出片厚度（校验 ≥280 词且含 camera/lighting/sound）。装箱在 [Shot 1] 演出节拍写 {{D1}} 占位、不得写 <d>；落库前把锁定开口台词与闭嘴内心填回原位，冻结留在最后一句之后，每句中文只出现一次。202 返回 job_id。重复提交同一镜进行中任务返回 duplicate。生成结果写入 beat.h3_prompt 与匹配后的 video_duration。出片对已保存提示词做同一套 prepare 再跑厚度校验，通过则复用。",
