...
OPERATION_DETAILS: dict[tuple[str, str], str] = {
    ...
    ("post", "/api/director/recipes/run"): "需要登录和 X-CSRF-Token。用现有 LLM Provider 顺序跑 9 Agent，写入 Recipe payload。可选 agents 只跑指定步骤（例如 script+storyboard 按剧本一次生成全部分镜）。运行中 payload.pipelineRun 记录本次步骤，agentStatus[].message 为人话阶段（读剧本/整理镜头），不回传模型思考原文。上游余额不足或欠费返回 HTTP 502，detail 含上游原文。可选 project_id 更新已有工程，否则新建。art_style_id 必须选自画风目录。",
    ...
    ("post", "/api/director/recipes/{project_id}/render-shots"): "需要登录和 X-CSRF-Token。按镜编译 ≤9 张参考图，并按 payload.videoWorkflowFamily 提交该族 T2V/I2V/R2V。有首帧或勾选上一镜尾帧时走 I2V/R2V。无定妆图且无首帧时走该族 T2V。缺省族为官方 MiniMax H3。",
