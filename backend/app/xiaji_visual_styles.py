...
VISUAL_STYLE_LABELS = {key: value["label"] for key, value in VISUAL_STYLE_PRESETS.items()}
...
def visual_style_label(style_id: str) -> str:
    ...
    return str(preset["label"]) if preset else ""
...
def settings_visual_style(settings: Any) -> str:
    payload = settings if isinstance(settings, dict) else {}
    raw = payload.get("visual_style")
    visual = normalize_visual_style(raw)
    if visual:
        return visual
    from .xiaji_art_style import first_art_style_id

    if first_art_style_id(raw):
        return ""
