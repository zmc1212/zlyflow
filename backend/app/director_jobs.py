    ...
    *,
    ...
    if job_id:
        ...
        if job:
            ...
            if file_path is not None:
                return file_path
    image_url = str(slot.get("previewUrl") or slot.get("imageUrl") or "")
    if image_url.startswith("/api/director/recipes/"):
        import urllib.parse
        parsed = urllib.parse.urlparse(image_url)
        parts = parsed.path.split("/")
        if len(parts) >= 8 and parts[3] == "recipes" and parts[5] == "asset-image":
            project_id = parts[4]
            kind = parts[6]
            asset_id = parts[7]
            look_id = urllib.parse.parse_qs(parsed.query).get("look_id", [None])[0]
            try:
                project = store.get_director_project(project_id)
                owner_user_id = str(project.get("owner_user_id") or "")
                if owner_user_id:
                    path = find_recipe_asset_image_file(
                        owner_user_id=owner_user_id,
                        project_id=project_id,
                        kind=kind,
                        asset_id=asset_id,
    ...
    if not library_id:
        library_id = parse_library_asset_id_from_url(image_url) or ""
    if library_id:
        try:
