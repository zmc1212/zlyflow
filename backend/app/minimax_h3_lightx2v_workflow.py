...
def build_minimax_h3_lightx2v_workflow(
    mode: JobMode,
    prompt: str,
    references: list[str],
    ...
    if mode not in LIGHTX2V_WORKFLOWS:
        raise ValueError(f"Unsupported LightX2V workflow: {mode}")

    width, height = h3_dimensions(options)
    is_reference_mode = mode is JobMode.MINIMAX_H3_LIGHTX2V_R2V
    ...
    sampler_name = str(options.get("sampler_name") or "euler")
    ...
    workflow: dict[str, dict[str, Any]] = {
        ...
        OUTPUT_NODE: {
            ...
            "inputs": {
                "video": ["13", 0],
                "filename_prefix": "video/ZLY_AI_VIDEO_STUDIO_LightX2V_MiniMax_H3",
                "format": "auto",
                "codec": "auto",
            },
        },
    ...
    if lora_strength:
        workflow["15"] = {
            ...
            "inputs": {
                "model": model_source,
                "lora_name": options.get("lora_name", LIGHTX2V_FL2V_4STEP_LORA),
