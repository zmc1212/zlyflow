"""资产生图 Prompt，对齐 source1/app/xiaji_asset_prompts.py。"""
...
def composed_style_line(visual_style: str = "", art_style_id: str = "") -> str:
    """风格(art_style_id)决定媒介合同；画风(visual_style)只叠加时代味道，不能把国漫打回写实古装剧。"""
    art = (art_style_id or "").strip()
    flavor_id = (visual_style or "").strip()
    visual_id = normalize_visual_style(art, flavor_id)
    parts: list[str] = []
    if visual_id:
        parts.append(visual_style_contract(visual_id))
        label = visual_style_label(visual_id)
        if label:
            parts.append(f"SELECTED ART STYLE / 风格: {label} ({visual_id}). Follow this medium strictly.")
    extra_art = ART_STYLE_EXTRA_HINTS.get(art, "")
    if extra_art:
        parts.append(extra_art)
    elif art and art != visual_id:
        parts.append(f"SELECTED ART STYLE / 风格: {art}.")
    flavor = VISUAL_FLAVOR_HINTS.get(flavor_id, "")
    if flavor:
        parts.append(f"SELECTED ERA/LOOK / 画风: {flavor_id}. {flavor}")
    elif flavor_id and flavor_id not in {visual_id, art}:
        parts.append(f"SELECTED ERA/LOOK / 画风: {flavor_id}.")
...
def character_look_prompt(
    ...
    *,
    ...
    visual_id = normalize_visual_style(
        style or extra.get("art_style_id") or "",
        visual_style or extra.get("visual_style") or "",
    ) or resolve_visual_style(visual_style or extra.get("visual_style") or "")
    visual = composed_style_line(visual_style or extra.get("visual_style") or "", style or extra.get("art_style_id") or "")
...
def _scene_asset(asset: dict[str, Any], extra: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "name": asset.get("name"),
        "definition": {
            "scene_type": extra.get("scene_type") or definition.get("scene_type") or "interior",
            "time_of_day": extra.get("time_of_day") or definition.get("time_of_day") or "",
            "environment_prompt": (
                extra.get("environment_prompt")
                or definition.get("environment_prompt")
            ...
            "description": extra.get("description") or definition.get("description") or asset.get("description") or "",
            "visual_style": extra.get("visual_style") or definition.get("visual_style") or "",
        },
    }
...
def scene_view_prompt(
    ...
    *,
    ...
    packed = _scene_asset(asset, extra)
    view = (view or "master").strip() or "master"
    if view == "reverse":
        return _scene_reverse_prompt(
            packed, extra or {}, style=style, visual_style=visual_style, has_master_reference=has_master_reference
        )
    if view in {"panorama", "pano"}:
    ...
    return _scene_front_prompt(packed, extra or {}, style=style, visual_style=visual_style)


def _scene_style_block(
    asset: dict[str, Any], extra: dict[str, Any], *, style: str = "", visual_style: str = ""
) -> str:
    style_line = composed_style_line(
        visual_style or extra.get("visual_style") or "",
        style or extra.get("art_style_id") or "",
    )
    if not style_line:
...
def _scene_front_prompt(asset: dict[str, Any], extra: dict[str, Any], *, style: str = "", visual_style: str = "") -> str:
    style_block = _scene_style_block(asset, extra, style=style, visual_style=visual_style)
...
def _scene_reverse_prompt(
    ...
    *,
    ...
    definition = asset.get("definition") if isinstance(asset.get("definition"), dict) else {}
    scene_type = str(definition.get("scene_type") or extra.get("scene_type") or "").strip().lower()
...
{_scene_style_block(asset, extra, style=style, visual_style=visual_style)}
...
def prop_view_prompt(
    ...
    *,
    ...
    view = (view or "master").strip() or "master"
    if view == "turnaround":
        return _prop_turnaround_prompt(
...
def _prop_asset(asset: dict[str, Any], extra: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {
        "name": asset.get("name"),
        "definition": {
            "prop_type": extra.get("prop_type") or definition.get("prop_type") or "object",
            "owner": extra.get("owner") or definition.get("owner") or asset.get("role") or "",
            "visual_prompt": extra.get("visual_prompt") or definition.get("visual_prompt") or asset.get("visual_prompt") or "",
            "description": extra.get("description") or definition.get("description") or asset.get("description") or "",
            "visual_style": extra.get("visual_style") or definition.get("visual_style") or "",
        },
    }
...
def _prop_style_block(asset: dict[str, Any], extra: dict[str, Any], *, style: str = "", visual_style: str = "") -> str:
    style_line = composed_style_line(
        visual_style or extra.get("visual_style") or "",
        style or extra.get("art_style_id") or "",
    )
...
def _prop_turnaround_prompt(
    ...
    *,
    ...
    if has_master_reference:
        ...
        style_block = _prop_style_block(asset, extra, style=style, visual_style=visual_style)
