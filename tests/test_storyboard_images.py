...
from server.services.episode_image_prompts import beat_sketch_prompt
from server.services.storyboard_image_service import StoryboardImageService, _ACTIVE
from server.services.project_detail_service import resolve_shot_scene
...
class StoryboardDispatcherTests(unittest.TestCase):
    ...
    @patch("server.services.storyboard_image_service._EXECUTOR")
    @patch("server.services.storyboard_image_service.execute_sql", return_value=1)
    @patch("server.services.storyboard_image_service.query_all")
